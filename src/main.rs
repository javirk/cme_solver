...
fn main() {
    ...
    simulation.add_species("A", n_a);
    simulation.add_species("B", n_b);
    simulation.add_species("C", n_c);
...
fn plot(simulation: Simulation) {
    ...
    let mut ctx = ChartBuilder::on(&root_area)
        ...
        .build_cartesian_2d(0..simulation.species_history["A"].len(), 0.0..1000.0)
        .unwrap();

    ctx.configure_mesh().draw().unwrap();

    ctx.draw_series(
        LineSeries::new((0..).zip(simulation.species_history["A"].iter().map(|x | *x as f64)), &GREEN)
    ).unwrap();
}